...
mFlags;const char*mObject;size_t mNumRanges;size_t mStart;}NeCERData;typedef
struct NeDsICRDataTag{const char*mFullPath;size_t mIndex;NeDsICRFlag mFlags;
const char*mObject;size_t mNumRanges;size_t mStart;}NeICRData;typedef struct
NeVariableDataTag{const char*mFullPath;size_t mIndex;NeDsVariableFlag mFlags;
const char*mObject;real_T mScale;const char*mUnit;real_T mICValue;boolean_T
mIsDifferential;boolean_T mHasInitialRange;size_t mM;size_t mN;NeInitMode
mInitMode;const char*mDescription;}NeVariableData;typedef struct
...
mDescription;size_t mIndex;size_t mM;boolean_T mIsTop;}NeParameterData;typedef
enum SscDiagnosticSettingTag{SSC_DIAGNOSTIC_INVALID= -1,SSC_DIAGNOSTIC_ERROR,
...
NESL_SIM_RESIDUALS,NESL_SIM_TOLERANCES,NESL_SIM_REINIT,
NESL_SIM_NUMJAC_DX_BOUNDS,NESL_NUM_SIMULATOR_METHODS}NeslSimulatorMethodId;
typedef struct NeslSimulatorTag NeslSimulator;typedef struct
NeslSimulatorUserDataTag NeslSimulatorUserData;typedef enum
...
NeDsExpandParamFcn mExpandParamFcn;NeDsRtpMapFcn mRtpMapFcn;NeChecksum
mChecksum;};PMF_DEPLOY_STATIC void ne_ds_release_reference(NeDynamicSystem*
dynSys){if(dynSys){dynSys->mReleaseReference(dynSys);}}PMF_DEPLOY_STATIC void
...
NeSolverParameters solverParameters,boolean_T isScalable,PmAllocator*
allocatorPtr);typedef struct NeConstraintStatTag NeConstraintStat;struct
...
mConstraint)(const NeCompStats*);struct NeCompStatsData*mData;};NeSolverSystem
