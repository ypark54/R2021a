...
static void* FMU2_fmuInitialize(const char* lib,
    ...
    fmi2Boolean isLoggingOn = fmi2False;   /*logging is currently disabled, on for debug*/
    char* fmuResLocation;
    void* diagnostic;
    
    fmustruct = (struct FMU2_CSME_RTWCG *)calloc(1, sizeof(struct FMU2_CSME_RTWCG));
    ...
    if (strlen(mccRes) == 0) {
        ...
        fmuResLocation = (char*) mccRes; /* use MCC dir, if provided */
    }
    fmustruct->callbacks.logger = fmu2Logger;
    fmustruct->callbacks.allocateMemory = calloc;
    fmustruct->callbacks.freeMemory = free;
    fmustruct->callbacks.stepFinished = NULL;
    fmustruct->callbacks.componentEnvironment = ssPtr; /* used in logger */
